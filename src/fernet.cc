...
using EVP_CIPHER_CTX_unique_ptr =
  typename std::unique_ptr<EVP_CIPHER_CTX, decltype(&::EVP_CIPHER_CTX_free)>;
using EVP_MD_CTX_unique_ptr =
  typename std::unique_ptr<EVP_MD_CTX, decltype(&::EVP_MD_CTX_free)>;
using EVP_MAC_CTX_unique_ptr =
  typename std::unique_ptr<EVP_MAC_CTX, decltype(&::EVP_MAC_CTX_free)>;
using EVP_MAC_unique_ptr =
  typename std::unique_ptr<EVP_MAC, decltype(&::EVP_MAC_free)>;
using OSSL_LIB_CTX_unique_ptr =
...
// This document describes version 0x80 (currently the only version) of the
// fernet format.
static byte constexpr FERNET_VERSION{ 128_uint8_t },
  CYPHER_BLOCK_SIZE{ 16_uint8_t }, VERSION_SIZE{ sizeof(byte) },
  TIME_SIZE{ sizeof(std::time_t) }, NONCE_SIZE{ 16_uint8_t },
  HMAC_SIZE{
    32_uint8_t
  }, /* = EVP_MD_get_size(EVP_MD_fetch(NULL, "SHA256", NULL)) */
  KEY_SIZE{ 32_uint8_t };

using byte_time = union [[nodiscard]] byte_time
{
  byte const* _timestamps_bytes_c;
  byte* _timestamps_bytes;
  int64_t* _timestamps;
  byte_time(int64_t* timestamps)
    : _timestamps(timestamps)
  {
  }
  byte_time(Fernet::secure_string::const_iterator const& token_it)
    : _timestamps_bytes_c(reinterpret_cast<byte const*>(token_it.base()))
  {
  }
  byte_time(char const* token_ptr)
    : _timestamps_bytes_c(reinterpret_cast<byte const*>(token_ptr))
  {
  }
};

[[nodiscard(
  "Must use calcDecodeLength return value")]] [[using gnu: always_inline,
                                                hot,
                                                access(read_only, 1)]]
...
[[nodiscard("Must use base64Decode return value")]] [[using gnu: always_inline,
                                                      hot,
                                                      access(read_only, 1)]]
...
base64Decode(Fernet::secure_string const& b64message) noexcept
{
  ...
  if (EVP_DecodeBlock(reinterpret_cast<byte*>(outbuffer.data()),
    ...
    return std::unexpected(std::format(
      "EVP_DecodeBlock not correctly decoded : can not decode the base64 of {}",
      b64message.c_str()));
  outbuffer.resize(decodeLen);
  return outbuffer;
}

[[nodiscard("Must use base64Encode return value")]] [[using gnu: always_inline,
                                                      hot,
                                                      access(read_only, 1)]]
static inline std::expected<Fernet::secure_string, std::string>
base64Encode(Fernet::secure_string const& input) noexcept
{
  ...
  using EVP_EncodeBlock_rt = std::invoke_result<decltype(&EVP_EncodeBlock),
                                                unsigned char*,
                                                const unsigned char*,
                                                int>::type;
...
[[nodiscard(
  "Must use urlsafe base64Encode return value")]] [[using gnu: always_inline,
                                                    hot,
                                                    access(read_only, 1)]]
static inline std::expected<Fernet::secure_string, std::string>
...
[[nodiscard(
  "Must use urlsafe base64 return value")]] [[using gnu: always_inline,
                                              hot,
                                              access(read_only, 1)]]
static inline std::expected<Fernet::secure_string, std::string>
...
[[nodiscard("Must use HMAC_SHA256 return value")]] [[using gnu: always_inline,
                                                     hot,
                                                     access(read_only, 1),
                                                     access(read_only, 2)]]
...
{
  std::array<byte, HMAC_SIZE> digest;

  // #if not defined(NDEBUG)
  //     uint32_t static const
  //     digest_length{static_cast<uint32_t>(EVP_MD_size(EVP_sha256()))};
  //     assert(HMAC_SIZE == digest_length && "HMAC_SIZE == digest_length must
  //     be equal");
  // #endif

  OSSL_LIB_CTX_unique_ptr library_context(OSSL_LIB_CTX_new(),
                                          ::OSSL_LIB_CTX_free);
  // Check if context creation failed
  if (not library_context) [[unlikely]] {
    // You might want to get OpenSSL error details here
    unsigned long err_code = ERR_get_error();
    std::string err_buf(256, '\0');
    ERR_error_string_n(err_code, err_buf.data(), err_buf.capacity());
    return std::unexpected(std::string("OSSL_LIB_CTX_new() failed: ") +
                           err_buf);
  }

  {
    EVP_MAC_unique_ptr mac_alg(
      EVP_MAC_fetch(library_context.get(), "HMAC", NULL), ::EVP_MAC_free);
    {
      EVP_MAC_CTX_unique_ptr mac_ctx(EVP_MAC_CTX_new(mac_alg.get()),
                                     ::EVP_MAC_CTX_free);
      std::string static digest_name{ "SHA256" };
      std::array const ossl_params{
        OSSL_PARAM_construct_utf8_string(
          OSSL_MAC_PARAM_DIGEST, digest_name.data(), 0),
      ...
      /* Make a call to the final with a NULL buffer to get the length of the
       * MAC
       */
...
[[nodiscard(
  "Must use AES_128_CBC_decrypt return value")]] [[using gnu: always_inline,
                                                   hot,
                                                   access(read_only, 1),
                                                   access(read_only, 2),
                                                   access(read_only, 3)]]
                    ...
                    Fernet::secure_string const& ctext) noexcept
{
  ...
  EVP_CIPHER_CTX_unique_ptr cipher_ctx(EVP_CIPHER_CTX_new(),
                                       ::EVP_CIPHER_CTX_free);
  int outlen{ 0 }, outlen2{ 0 };

  if (not EVP_DecryptInit_ex(cipher_ctx.get(),
                             EVP_aes_128_cbc(),
                             NULL,
                             reinterpret_cast<byte const*>(key.data()),
...
[[nodiscard(
  "Must use AES_128_CBC_encrypt return value")]] [[using gnu: always_inline,
                                                   hot,
                                                   access(read_only, 1),
                                                   access(read_only, 2),
                                                   access(read_only, 3)]]
                    ...
                    Fernet::secure_string const& ptext) noexcept
{
  ...
  EVP_CIPHER_CTX_unique_ptr const ctx(EVP_CIPHER_CTX_new(),
                                      ::EVP_CIPHER_CTX_free);
  int outlen{ 0 }, outlen2{ 0 };

  // #if not defined(NDEBUG)
  // static uint64_t const
  // cipher_block_size{static_cast<uint64_t>(EVP_CIPHER_block_size(EVP_aes_128_cbc()))};
  // assert(cipher_block_size == CYPHER_BLOCK_SIZE && "cipher_block_size ==
  // CYPHER_BLOCK_SIZE must be equal"); #endif

  if (not EVP_EncryptInit_ex2(ctx.get(),
                              EVP_aes_128_cbc(),
                              reinterpret_cast<byte const*>(key.data()),
                              reinterpret_cast<byte const*>(nonce.data()),
                              NULL)) [[unlikely]]
  ...
  uint64_t const cipher_text_len{ ptext.size() + CYPHER_BLOCK_SIZE -
                                  (ptext.size() % CYPHER_BLOCK_SIZE) };
...
[[nodiscard(
  "Must use timestamp_byteswap return value")]] [[using gnu: always_inline,
                                                  hot,
                                                  access(read_only, 1)]]
...
Fernet::Fernet(Fernet::secure_string const& key) noexcept
  : _key(urlsafe_base64Decode(key).value_or("0000000000000000000000000000000"))
{
}

[[nodiscard("Must use gen_rand_key return value")]] [[using gnu: hot]]
...
Fernet::gen_rand_key() noexcept
{
  ...
  if (not RAND_bytes(reinterpret_cast<byte*>(keys.data()), KEY_SIZE))
    [[unlikely]]
    return std::unexpected("RAND_bytes for iv failed");
  return urlsafe_base64Encode(
    /* Fernet::secure_string const &input */ { keys.data(), keys.size() });
}

[[nodiscard("Must use encrypt return value")]] [[using gnu: hot,
                                                 access(read_only, 1)]]
std::expected<Fernet::secure_string, std::string>
                ...
                std::time_t const current_time) const noexcept
{
  ...
  FERNET_ASSIGN_OR_RAISE(
    int64_t timestamp,
    timestamp_byteswap(/* int64_t const &timestamp */ current_time))

  byte_time const timestamp_view(&timestamp);

  Fernet::secure_string token;
  token.reserve(VERSION_SIZE + TIME_SIZE + NONCE_SIZE +
                (plain_text.size() + NONCE_SIZE) + HMAC_SIZE);

  token.resize(VERSION_SIZE, '\0');
  token[0] = FERNET_VERSION;

  std::span<const byte, TIME_SIZE> const timestamp_range_view{
    timestamp_view._timestamps_bytes_c, TIME_SIZE
  };
  std::copy_n(
    timestamp_range_view.begin(), TIME_SIZE, std::back_inserter(token));
  ...
  FERNET_ASSIGN_OR_RAISE(
    auto const& cypher_text,
    AES_128_CBC_encrypt(/* key  */ { enc_key_view.data(), enc_key_view.size() },
                        /* nonce  */ nonce,
                        /* ptext  */ plain_text))
...
[[nodiscard("Must use decrypt return value")]] [[using gnu: hot,
                                                 access(read_only, 1)]]
std::expected<Fernet::secure_string, std::string>
                ...
                std::time_t current_time) const noexcept
{
  ...
  FERNET_ASSIGN_OR_RAISE(
    ...
    timestamp_byteswap(
      /* int64_t const &timestamp */ *token_timestamp._timestamps))

  if (ttl and (timestamp + ttl) < current_time)
    return std::unexpected("TTL Expired");

  Fernet::secure_string const token_data{ std::string_view(
    decoded_token.begin(), decoded_token.end() - HMAC_SIZE) };

  auto const hmac_key_view{ this->_key | std::views::take(16) };

  FERNET_ASSIGN_OR_RAISE(
    auto const& hmac_sha256_computed,
    HMAC_SHA256(/* Fernet::secure_string const &data */ token_data,
                /* Fernet::secure_string const &key */ {
                  hmac_key_view.data(), hmac_key_view.size() }))

  auto const hash_recieved_view{
    decoded_token | std::views::drop(decoded_token.size() - HMAC_SIZE)
  };
  if (not std::ranges::equal(hmac_sha256_computed,
                             hash_recieved_view,
                             [](byte const left, byte const right) -> bool {
                               return left == right;
                             })) [[unlikely]]
    return std::unexpected(
      std::format("hash_computed are not eq to the hash_recieved "));

  Fernet::secure_string const cypher_text{ std::string_view(
    decoded_token.begin() + VERSION_SIZE + TIME_SIZE + NONCE_SIZE,
    decoded_token.end() - HMAC_SIZE) };

  auto const enc_key_view{ this->_key | std::views::drop(16) };
  return AES_128_CBC_decrypt(
    /* Fernet::secure_string const &key */ { enc_key_view.data(),
                                             enc_key_view.size() },
