...
#define FERNET_ASSIGN_OR_RAISE(definition, expression)                         \
  FERNET_ASSIGN_OR_RAISE_IMPL(                                                 \
    FERNET_ASSIGN_OR_RAISE_NAME(_error_or_value_, __COUNTER__),                \
    definition,                                                                \
    expression)

// Fernet is a symmetric encryption method which makes sure that the message
// encrypted cannot be manipulated/read without the key. It uses URL safe
// encoding for the keys. Fernet also uses 128-bit AES in CBC mode and PKCS7
// padding, with HMAC using SHA256 for authentication. The IV is created from
// openssl RAND_bytes. All of this is the kind of thing that good software
// needs. AES is top drawer encryption, and SHA-256 avoids many of the problems
// caused by MD5 and SHA-1 (as the length of the hash values is too small). With
// CBC (Cipher Block Chaining) we get a salted output, and which is based on a
// random value (the IV value). And with HMAC we can provide authenticated
// access from both sides. In this case we will use scrypt to generate the
// encryption key from a salt value and a password. To generate the same
// encryption key we need the salt value and the password
...
typedef struct [[nodiscard]] Fernet final
{
  ...
  public:
    ...
    pointer allocate(size_type n) const
    {
      ...
      return static_cast<pointer>(
        OPENSSL_secure_zalloc(n * sizeof(value_type)));
    }
    ...
    pointer operator=(pointer p)
    {
      if (std::size(p) > std::numeric_limits<size_type>::max() / sizeof(T))
        throw std::bad_alloc();
      return static_cast<pointer>(
        OPENSSL_secure_zalloc(std::size(p) * sizeof(value_type)));
    }
  ...
  using secure_string =
    std::basic_string<char, std::char_traits<char>, zallocator<char>>;
...
public:
  ...
  [[nodiscard("Must use encrypt return value")]] [[using gnu: hot,
                                                   access(read_only, 1)]]
  std::expected<secure_string, std::string> encrypt(
    secure_string const& plain_text,
    std::time_t current_time = std::time(nullptr)) const noexcept;

  [[nodiscard("Must use decrypt return value")]] [[using gnu: hot,
                                                   access(read_only, 1)]]
  std::expected<secure_string, std::string> decrypt(
    secure_string const& token,
    int64_t ttl = 0U,
    std::time_t current_time = std::time(nullptr)) const noexcept;
...
[[using gnu: hot, access(read_only, 1), access(read_only, 2)]]
constexpr bool
operator==(const Fernet::zallocator<T>&, const Fernet::zallocator<U>&) noexcept
{
  return true;
}
...
[[using gnu: hot, access(read_only, 1), access(read_only, 2)]]
constexpr bool
operator!=(const Fernet::zallocator<T>& lhs,
           const Fernet::zallocator<U>& rhs) noexcept
{
